...
\begin{enumerate}
    \item \textbf{Filtering Label}: Data difilter untuk hanya menyisakan baris dengan label DDoS dan BENIGN, karena kedua kelas inilah yang menjadi fokus klasifikasi biner dalam penelitian ini. Kelas lain seperti serangan selain DDoS tidak digunakan agar model yang dihasilkan bersifat spesifik untuk deteksi DDoS.

    \item \textbf{Penanganan Nilai Tak Hingga}: Nilai tak hingga ($\pm\infty$) yang muncul akibat pembagian dengan nol pada proses ekstraksi CICFlowMeter diubah menjadi nilai kosong (\textit{Not a Number} atau NaN) untuk memudahkan proses penanganan data kosong selanjutnya.

    \item \textbf{Penghapusan Fitur dengan Banyak Data Kosong}: Fitur yang memiliki proporsi data kosong lebih dari 50\% dihapus dari dataset. Hal ini dilakukan karena fitur dengan banyak nilai hilang dianggap tidak reliabel dan dapat menurunkan kualitas model jika dipertahankan.

    \item \textbf{Imputasi Data Kosong}: Nilai kosong yang tersisa pada fitur-fitur lainnya diisi menggunakan nilai median dari kolom fitur tersebut. Median dipilih karena statistik ini lebih tangguh (\textit{robust}) terhadap anomali ekstrem dibandingkan rata-rata, sehingga lebih cocok untuk data lalu lintas jaringan yang sering mengandung nilai pencilan (\textit{outliers}).

    \item \textbf{Normalisasi Data}: Seluruh fitur numerik dinormalisasi menggunakan \texttt{StandardScaler} dari pustaka Scikit-learn. Proses ini mengubah setiap fitur sehingga memiliki nilai rata-rata 0 dan standar deviasi 1, yang bertujuan menyamakan skala antar fitur agar tidak ada fitur dengan rentang nilai besar yang mendominasi proses pembelajaran model.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Akurasi (\textit{Accuracy})}: Persentase prediksi benar dari keseluruhan data uji.
    \item \textbf{Presisi (\textit{Precision})}: Proporsi prediksi DDoS yang benar terhadap seluruh prediksi DDoS, mengukur kemampuan model dalam meminimalkan \textit{false positive}.
    \item \textbf{\textit{Recall}}: Proporsi data DDoS yang berhasil dideteksi dengan benar, mengukur kemampuan model dalam meminimalkan \textit{false negative}.
    \item \textbf{F1-Score}: Rata-rata harmonik dari Presisi dan \textit{Recall}, yang menjadi metrik acuan utama karena memberikan evaluasi yang lebih komprehensif dibandingkan akurasi saja, terutama pada data dengan ketidakseimbangan kelas.
    \item \textbf{\textit{False Positive Rate} (FPR)}: Persentase data normal yang salah terdeteksi sebagai DDoS.
    \item \textbf{AUC-ROC}: Area di bawah kurva ROC yang mengukur kemampuan model dalam membedakan kedua kelas secara keseluruhan.
    \item \textbf{Waktu Pelatihan (\textit{Training Time})}: Waktu yang dibutuhkan untuk melatih model, diukur dalam detik.
    \item \textbf{Waktu Inferensi (\textit{Inference Time})}: Waktu yang dibutuhkan untuk memprediksi satu aliran data, diukur dalam milidetik per \textit{flow}.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Analisis Offline}: Pengguna mengunggah berkas tangkapan jaringan dalam format \texttt{.pcap} atau \texttt{.csv}, kemudian aplikasi melakukan ekstraksi aliran, klasifikasi, dan menampilkan hasil deteksi pada dasbor web.
    \item \textbf{Penangkapan Paket Real-Time} (\textit{Live Sniffing}): Aplikasi menangkap paket secara langsung dari antarmuka jaringan menggunakan pustaka Scapy untuk durasi tertentu, kemudian melakukan analisis secara langsung.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Pengujian Fungsional}: Aplikasi diuji menggunakan berkas \texttt{.pcap} yang telah diketahui isinya (mengandung jejak serangan DDoS) untuk memverifikasi kemampuan ekstraksi aliran, klasifikasi, dan visualisasi forensik. Hasil deteksi dibandingkan dengan analisis manual menggunakan Wireshark sebagai \textit{ground truth}.
    \item \textbf{Pengujian Performa}: Aplikasi diuji menggunakan tiga ukuran berkas \texttt{.pcap} (100 flow, 1.000 flow, dan 10.000 flow) untuk mengukur latensi pemrosesan, \textit{throughput} model, penggunaan CPU, dan penggunaan memori RAM. Pengukuran dilakukan dalam kondisi server idle dan saat memproses data.
\end{itemize}
